            ...
            onSuccess = {
                if (user.photo == null) {
                    telegramApiExecutor.sendMessage(
                        chatId = adminId,
                        text = "${user.name}(@${user.username}) подал заявку на вступление. Если это так, то нажмите \"Подтвердить\". Если в течение нескольких минут заявка на вступление не пришла, то просто проигнорируйте это сообщение",
